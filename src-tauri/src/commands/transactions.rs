...
pub fn create_sale(state: tauri::State<'_, AppState>, input: CreateSaleInput) -> Result<Transaction, String> {
    ...
    crate::activity_helper::log_activity(&db, crate::activity_helper::ActivityEntry {
        ...
        entity_id: Some(&id),
        description: &format!("بيع فستان {} للعميل {} بسعر {} {}", dress_code, customer_name, input.price,
            match sale_currency.as_str() { "USD" => "$", "TRY" => "₺", _ => "ل.س" }),
    ...
    Ok(Transaction {
        ...
        employee_id: input.employee_id, pickup_date: input.pickup_date, notes: input.notes,
...
pub fn create_rental(state: tauri::State<'_, AppState>, input: CreateRentalInput) -> Result<Transaction, String> {
    ...
    Ok(Transaction {
        ...
        return_date: None, employee_id: input.employee_id, pickup_date: None, notes: input.notes,
